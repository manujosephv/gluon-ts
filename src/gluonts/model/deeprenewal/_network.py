...
class DeepRenewalNetwork(mx.gluon.HybridBlock):
    @validated()
    def __init__(
        self,
        num_layers: int,
    ...
    def unroll_encoder(
        ...
        # _, scale_q = self.scaler(
        #     past_target[:,:,1].slice_axis(axis=1, begin=-self.context_length, end=None),
        #     past_observed_values.slice_axis(
        #         axis=1, begin=-self.context_length, end=None
        #     ),
        # )
        ...
        # (batch_size, sub_seq_len, *target_shape, num_lags)
        lags_scaled_m = F.broadcast_div(lags_m, scale.expand_dims(axis=-1))
        lags_scaled_q = F.broadcast_div(lags_q, scale.expand_dims(axis=-1))
...
class DeepRenewalTrainingNetwork(DeepRenewalNetwork):
    ...
    def hybrid_forward(
        ...
        weighted_loss = weighted_average(F=F, x=loss, weights=loss_weights, axis=1)
...
class DeepRenewalPredictionNetwork(DeepRenewalNetwork):
    ...
    def sampling_decoder(
        ...
        for k in range(self.prediction_length):
            ...
            # (batch_size * num_samples, 1, *target_shape, num_lags)
            lags_scaled_m = F.broadcast_div(lags_m, repeated_scale.expand_dims(axis=-1))
            lags_scaled_q = F.broadcast_div(lags_q, repeated_scale.expand_dims(axis=-1))
